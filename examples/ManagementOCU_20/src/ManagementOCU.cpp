...
#include "ManagementOCU.h"
#include "JausUtils.h"

using namespace JTS;
using namespace urn_jts_example_management_ocu_1_0;


ManagementOCU::ManagementOCU(unsigned int subsystem, unsigned short node, unsigned short component)
{
	jausRouter = new JausRouter(JausAddress(subsystem, node, component), ieHandler);
	
	/// Instantiate services
	ManagementOCUService* pManagementOCUService = new ManagementOCUService(jausRouter);
	
	
	/// Add all the Services for the Component
	serviceList.push_back(pManagementOCUService);
	
}

ManagementOCU::~ManagementOCU()
{
	Service* service;
...
void ManagementOCU::startComponent()
{
	Service* service;
...
void ManagementOCU::shutdownComponent()
{
	Service* service;
...
void ManagementOCU::processInternalEvent(InternalEvent *ie)
{
    bool done = false;
