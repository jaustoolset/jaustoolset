...
import src.urn_jts_example_ocu_1_0.OCUService;



class ExampleOCU extends EventReceiver
    {
	protected ArrayList<Service> serviceList = new ArrayList<Service>();
	JausRouter jausRouter;

	ExampleOCU(int subsystem, short node, short component)
    {
		jausRouter = new JausRouter(new JausAddress(subsystem, node, component), ieHandler);
		
	/// Instantiate services
		OCUService pOCUService = new OCUService(jausRouter);


	/// Add all the Services for the Component
		serviceList.add(pOCUService);
