...
<!-- 
  ...
  However, there are also some non-Docbook elements output, such as <font-resize>,
  that are intended to be consumed by docbook_cals_fo_word.xsl to produce custom XSL-FO
  output that is eventually rendered as a Word document by XMLMind XFC.
...
<xsl:stylesheet
  ...
  <!--
      This transform turns JSIDL service definitions, one service_def per file,
      into Docbook format.   Docbook structure supports further transformation
	  into XSL-FO, for final rendering as a Word document in SAE documentation
	  format.
  -->
  ...
  <xsl:template name="make-vocabulary-table">
    ...
    <xsl:if test="($num_input_message_defs + $num_output_message_defs) > 0">
      <!-- if calling context is correct this should give the service def's name -->
        ...
        <tgroup cols="3" align="left" colsep="1" rowsep="1">
          ...
          <tbody>
            ...
            <xsl:if test="$num_input_message_defs > 0">
              ...
              <xsl:for-each select="//jaus:input_set/jaus:message_def">
            ...
            <xsl:if test="$num_output_message_defs > 0">
              ...
              <xsl:for-each select="//jaus:output_set/jaus:message_def">
                <xsl:variable name="link-text">
                  <!-- computes xml:id of event def to link to. -->
                  <xsl:value-of select="$service_name"/>
                  <xsl:text>.</xsl:text>
      ...
      <table>
        ...
        <tgroup cols="2" align="left" colsep="1" rowsep="1">
          ...
          <tbody>
            ...
            <xsl:for-each select="//jaus:internal_events_set/jaus:event_def">
              <xsl:variable name="link-text">
                <!-- computes xml:id of event def to link to. -->
                <xsl:value-of select="$service_name"/>
                <xsl:text>.</xsl:text>
  ...
  <xsl:template name="make-state-transition-table">
    ...
    <xsl:variable name="num_transitions" 
      select="count(//jaus:state/jaus:transition | //jaus:default_state/jaus:transition)"/>
    <table>
  ...
  <xsl:template match="jaus:event_def">
    <xsl:if test="not (@type = 'internal')">
      <!-- compute a unique xml:id with service def name and event def name. -->
  ...
  <xsl:template match="jaus:message_def">
    <xsl:if test="not (@type = 'internal')">
      <!-- compute a unique xml:id with service def name and message def name. -->
