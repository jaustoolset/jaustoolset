...
Export('env')
SConscript([common_dir + '/Sconstruct', common_dir + '/libCSharp/libCommon_CSharp/framework/%transport_version%/Sconstruct'])
...
build_first = SConscript([%build_list%])
	
libs = ['statemap', 'CSharpCommon' %service_libs%]
pathlibs = env.CLIRefs(libs)

prog = env.CLIProgram('./bin/%file_name%', sources, ASSEMBLYREFS=pathlibs)
