/**
 * $Id: mxEdgeStyle.java,v 1.27 2009/02/20 14:25:23 gaudenz Exp $
...
public class mxEdgeStyle
{
	...
	public static mxEdgeStyleFunction EntityRelation = new mxEdgeStyleFunction()
	{
		...
		public void apply(mxCellState state, mxCellState source,
				mxCellState target, List points, List result)
		{
	...
	public static mxEdgeStyleFunction Loop = new mxEdgeStyleFunction()
	{
				...
				mxCellState target, List points, List result)
		{
			...
			mxPoint pt = (points != null && points.size() > 0) ? (mxPoint) points.get(0) : null;
			...
			if (pt != null)
			{
				...
				pt = new mxPoint(s * (t.getX() + pt.getX() + o.getX()), s
						* (t.getY() + pt.getY() + o.getY()));
	...
	public static mxEdgeStyleFunction SideToSide = new mxEdgeStyleFunction()
	{
		...
		{
			mxGraphView view = state.getView();
			mxPoint pt = (mxPoint) ((points != null && points.size() > 0) ? points.get(0) : null);
			...
			if (source == null)
			{
				mxPoint tmp = (mxPoint) state.absolutePoints.get(0);
			...
			{
				List pts = state.absolutePoints;
				mxPoint tmp = (mxPoint) pts.get(pts.size() - 1);
