...
public class mxResizeManager extends mxEventSource
{
	 ...
	 * 
	 */
	public mxResizeManager(mxGraph graph)
	{
		setGraph(graph);
	}
	...
	public void setGraph(mxGraph graph)
	{
		...
		if (this.graph != null)
		{
			this.graph.addListener(mxGraph.EVENT_RESIZE, resizeHandler);
			this.graph.addListener(mxGraph.EVENT_FOLD, resizeHandler);
		}
	}
					...
					for (int i = 0; i < cells.length; i++)
					{
						if (cells[i] != cell && isCellShiftable(cells[i]))
						{
							shiftCell(
									...
									isExtendParents()
											&& graph
													.isExtendParentOnResize(cells[i]));
						}
					}
				}
				finally
