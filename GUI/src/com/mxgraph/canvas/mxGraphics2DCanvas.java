/**
 * $Id: mxGraphics2DCanvas.java,v 1.142 2009/02/15 10:01:06 gaudenz Exp $
...
public class mxGraphics2DCanvas extends mxBasicCanvas
{
	...
	{
		Image img = (Image) imageCache.get(image);
			...
			Hashtable style, boolean isHtml)
	{
		if (g != null && drawLabels)
		{
			...
			if (label != null && label.length() > 0)
			{
				...
				if (isHtml)
				{
					drawHtmlText(mxUtils.getBodyMarkup(label, true), x, y, w,
							h, style);
				}
				else
				{
	 ...
	 */
	public void drawShape(int x, int y, int w, int h, Hashtable style)
	{
		...
		if (g.hitClip(x - pw, y - pw, w + 2 * pw, h + 2 * pw))
		{
            ...
            // JTS
            else if (shape.equals(mxConstants.SHAPE_CIRCLE))
			{
                if( w > h) {
                  drawCircle(x, y, w, fillColor, fillPaint, penColor, shadow);
                    h=w;

                }
                else {
                  drawCircle(x, y, h, fillColor, fillPaint, penColor, shadow);
                    w=h;
                }
			}
            // jts
    ...
    /**
     * JTS
	 * Draws a circle for the given parameters.
	 *
	 * @param x X-coordinate of the shape.
	 * @param y Y-coordinate of the shape.
	 * @param r radius
	 ...
	 * @param shadow Boolean indicating if a shadow should be painted.
	 */
	protected void drawCircle(int x, int y, int r, Color fillColor,
			Paint fillPaint, Color penColor, boolean shadow)
	{
		if (fillColor != null || fillPaint != null)
		{
			if (shadow)
			{
				g.setColor(mxConstants.SHADOW_COLOR);
				g.fillOval(x + mxConstants.SHADOW_OFFSETX, y
						+ mxConstants.SHADOW_OFFSETY, r, r);
			}
			...
			float strokeWidth)
	{
        ...
        // JTS
        else if (type.equals(mxConstants.ARROW_RECTANGLE))
		{
           nx *= 1.2;
			ny *= 1.2;
            ...
            poly.addPoint((int) Math.round(pe.getX() + pOffset / 2), (int) Math
					.round(pe.getY() + pOffset / 2));
            poly.addPoint( (int) Math.round(pe.getX() - pOffset / 2) ,(int) Math
					.round(pe.getY() + pOffset / 2) );
            poly.addPoint( (int) Math.round(pe.getX() - pOffset / 2), (int) Math
					.round(pe.getY() - pOffset / 2));
