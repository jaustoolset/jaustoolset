/**
 * $Id: mxCellMarker.java,v 1.7 2009/02/17 15:33:25 gaudenz Exp $
...
public class mxCellMarker extends JComponent
{
	...
	{
		if (isHotspotEnabled() && hotspot > 0)
		{
			int x = (int) Math.round(state.getCenterX());
			int y = (int) Math.round(state.getCenterY());
			...
			if (start > 0)
			{
				...
				{
					y = (int) Math.round(state.getY() + start / 2);
					height = start;
				}
				else
				{
					x = (int) Math.round(state.getX() + start / 2);
					width = start;
				}
			}
			...
			Rectangle rect = new Rectangle((int) Math.round(x - w / 2),
					(int) Math.round(y - h / 2), w, h);
