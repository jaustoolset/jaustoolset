/**
 * $Id: mxConnectionHandler.java,v 1.11 2009/02/25 16:35:20 gaudenz Exp $
...
public class mxConnectionHandler extends mxMouseControl
{
	...
	public mxConnectionHandler(mxGraphComponent graphComponent)
	{
		...
		view.addListener(mxGraphView.EVENT_SCALE, resetHandler);
		view.addListener(mxGraphView.EVENT_TRANSLATE, resetHandler);
		view.addListener(mxGraphView.EVENT_SCALE_AND_TRANSLATE, resetHandler);

		mxIGraphModel model = graphComponent.getGraph().getModel();
		model.addListener(mxIGraphModel.EVENT_CHANGE, resetHandler);
		...
		if (target != null || graph.isAllowDanglingEdges())
		{
			...
			try
			{
				...
				if (newTarget != null)
				{
					...
					graph.addCell(newTarget, dropTarget);
				}
	...
	public void mousePressed(MouseEvent e)
	{
		...
		if (!e.isConsumed() && source != null && !e.isPopupTrigger())
		{
			...
			//		|| (!isHighlighting() && getBounds().contains(e.getPoint())))
			if ( graphComponent.isDragTriggerDesired() )
			// jts
			{
				start = e.getPoint();
